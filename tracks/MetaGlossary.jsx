/* global React, TopicPage, TPTitle, TPLede, TPSection, TPBody, Badge, Chip, Tex */
// Part III · Topic 13 — Glossary & Notation
// Single-track meta page. The reference desk.

function MetaGlossary() {
  const topic = {
    n: "13",
    part: "III",
    partName: "Meta",
    ...
    prev: { n: "12", title: "What This Is Not" },
    next: { n: "14", title: "Paper Library" },
  };
...
function NotationRow({ row }) {
  ...
  React.useEffect(() => {
    if (texRef.current && window.katex) {
      try {
        window.katex.render(row.sym, texRef.current, { throwOnError: false, displayMode: false });
      } catch (e) { texRef.current.textContent = row.sym; }
    }
  }, [row.sym]);
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 18, color: "var(--ink-1)" }} ref={texRef} />
