/* global React */
// Artboard 2 — Track-choice home (reworked hero)

function TrackChoiceArtboard() {
  return (
      ...
      <section style={{ padding: "90px 0 60px", position: "relative" }}>
        <div className="fu-wrap">
          ...
          <div style={{ maxWidth: 860, marginBottom: 72 }}>
            ...
            <h1 className="fu-display fu-h1" style={{ marginBottom: 28, fontWeight: 280, fontSize: "clamp(48px, 6.5vw, 92px)" }}>
              Physics, <em style={{
          ...
          <div style={{
            ...
            <span className="fu-eyebrow" style={{ color: "var(--accent)" }}>◇ choose a track</span>
              ...
              sample={{
                ...
                body: (
                  <>
                    ...
                    the bulk <Tex tex="\mathbb{R}^{3}\!\times\!\mathbb{R}_w\!\times\!\mathbb{R}_t" />,
                    with a stiff polytropic EoS <Tex tex="P(\rho)=K\rho^{n}" />,
                    <Tex tex="\,n=5" />. A brane observer at <Tex tex="w=0" /> measures…
                  </>
                ),
              }}
          ...
          <div style={{
            ...
            <span className="fu-mono" style={{
              fontSize: 11, letterSpacing: "0.14em", color: "var(--ink-3)", textTransform: "uppercase",
            }}>
              ◇ either track
            </span>
            <span className="fu-small" style={{ color: "var(--ink-2)", maxWidth: 680 }}>
              Every page has a link to the other track's version, so you can switch mid-read
              without losing your place. Prefer to browse the raw source? The{" "}
              <a className="fu-link">Paper Library</a> indexes all 8 .md source documents.
            </span>
          </div>

          {/* Constants — kept from original hero, hairline grid */}
          <div style={{
            marginTop: 64,
            display: "grid",
            ...
            {[
              ...
              <div key={k} style={{ padding: "20px 22px", background: "rgba(16,20,42,0.5)" }}>
  ...
  return (
    <nav style={{
      ...
      <div className="fu-wrap" style={{ display: "flex", alignItems: "center", height: 68, gap: 28 }}>
        ...
        <span className="fu-small" style={{ marginLeft: 4, opacity: 0.5 }}>v0.5</span>
        <div style={{ flex: 1 }} />
        {items.map((x, i) => (
...
function TrackCard({ kind, color, gradient, title, tagline, audience, bullets, sample, cta }) {
  return (
    <div style={{
      position: "relative",
      ...
      <div className="fu-mono" style={{
        fontSize: 11, letterSpacing: "0.14em", color, marginBottom: 12, textTransform: "uppercase",
      }}>
        ◇ {kind} track
      </div>
      <h2 className="fu-display" style={{
        fontSize: 40, color: "var(--ink-1)", fontWeight: 300, marginBottom: 8, lineHeight: 1.05,
      }}>{title}</h2>
      <div className="fu-body" style={{
        color: "var(--ink-2)", fontSize: 17, fontStyle: "italic", marginBottom: 20,
      }}>{tagline}</div>

      <div className="fu-small" style={{
        color: "var(--ink-3)", marginBottom: 22, letterSpacing: "0.02em",
      }}>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", marginRight: 8, opacity: 0.8 }}>for</span>
        {audience}
      </div>

      <ul style={{
        listStyle: "none", padding: 0, margin: "0 0 26px 0",
        display: "flex", flexDirection: "column", gap: 12,
      }}>
        {bullets.map((b, i) => (
          <li key={i} className="fu-body" style={{ fontSize: 14.5, color: "var(--ink-2)", paddingLeft: 20, position: "relative" }}>
            <span style={{
      ...
      <div style={{
        ...
        <div className="fu-mono" style={{
          fontSize: 10, letterSpacing: "0.14em", color: "var(--ink-3)", textTransform: "uppercase", marginBottom: 8,
        }}>
          ◇ {sample.label}
        </div>
        <div className="fu-body" style={{
          fontSize: 14, color: "var(--ink-1)", fontStyle: kind === "plain" ? "italic" : "normal", lineHeight: 1.6,
        }}>
          {sample.body}
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-start", gap: 12 }}>
        <a className="fu-mono" style={{
          ...
          background: color, color: "#0a0d18", fontWeight: 600, cursor: "pointer",
        }}>{cta}</a>
      </div>
    </div>
  );
...
Object.assign(window, { TrackChoiceArtboard });
